...
export async function GET() {
  ...
  const terms = await prisma.term.findMany({ include: { activeClassGroups: true, gameModules: true }, orderBy: { createdAt: 'desc' } });
...
export async function POST(request: NextRequest) {
  ...
  const { name } = body;
  if (!name) return NextResponse.json({ error: '請提供 name' }, { status: 400 });
