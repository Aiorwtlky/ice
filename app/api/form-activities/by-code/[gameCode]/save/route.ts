...
export async function POST(
  ...
  try {
    ...
    if (!user || user.role !== UserRole.STUDENT) {
      return NextResponse.json({ error: '只有學生可以暫存作答' }, { status: 403 });
    }
    ...
    await prisma.formAnswerLog.create({
      data: {
        ...
        detail: {
          source: 'student',
          currentQuestionId:
            typeof body.currentQuestionId === 'string' ? body.currentQuestionId : null,
    ...
    console.error('POST /api/form-activities/by-code/[gameCode]/save:', error);
