        ...
        <div className="mx-auto grid w-full max-w-7xl h-full min-h-0 gap-6 lg:grid-cols-12 items-stretch">
          ...
          <div className="rounded-2xl border border-gray-200/60 bg-white/90 p-6 shadow-lg backdrop-blur-sm min-h-0 flex flex-col lg:col-span-8">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto pr-1">
              ...
              {tab === 'terms' && <TermsTab terms={terms} onMutate={mutateTerms} />}
...
function TermsTab({ terms, onMutate }: { terms: { id: string; name: string }[]; onMutate: () => void }) {
  const [name, setName] = useState('');
  const [editId, setEditId] = useState<string | null>(null);
  const [editName, setEditName] = useState('');
  ...
  const ensure321 = async () => {
    // 1) 確保分類存在
    let term = terms.find((t) => t.name === '3/21資訊教育');
    if (!term) {
      const r = await fetch('/api/admin/terms', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: '3/21資訊教育' }) });
      const d = await r.json();
      if (!r.ok) { alert(d.error || '建立分類失敗'); return; }
      await onMutate();
      term = d.term ? { id: d.term.id, name: d.term.name } : undefined;
    }
    // 重新抓一次（避免上面沒有拿到 term.id）
    if (!term) {
      const r2 = await fetch('/api/admin/terms');
      const d2 = await r2.json();
      term = (d2.terms ?? []).find((t: { id: string; name: string }) => t.name === '3/21資訊教育') ?? null;
    }
