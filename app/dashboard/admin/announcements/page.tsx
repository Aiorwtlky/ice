...
export default function AdminAnnouncementsPage() {
  ...
  const { data: allData, mutate: mutateAll } = useSWR('/api/admin/announcements', fetcher);
  const listUrl = classId ? `/api/teacher/announcements?classGroupId=${classId}` : null;
  const { data: listData, mutate: mutateClass } = useSWR(listUrl, fetcher);

  useEffect(() => {
    sendLog('ANNOUNCEMENT_ADMIN_PAGE_VIEW', { payload: {} });
  }, [sendLog]);
  ...
  useEffect(() => {
    if (readsFor) sendLog('ANNOUNCEMENT_READS_VIEW', { payload: { announcementId: readsFor } });
  }, [readsFor, sendLog]);

  const announcements = listData?.announcements ?? [];
  const allRows = allData?.announcements ?? [];

  useEffect(() => {
    if (!meData?.user) return;
    if (meData.user.role !== 'ADMIN') router.replace('/dashboard');
  }, [meData, router]);
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="rounded-3xl border border-indigo-200 bg-white p-6 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">班級公告（管理員）</h1>
              <p className="text-sm text-gray-600">選擇任一班級發布；學生端與老師端共用同一套資料與已讀紀錄。</p>
            </div>
          </div>

          <div className="mt-6">
            <label className="text-xs font-bold text-gray-500">發布至班級</label>
            <select
              value={classId}
              onChange={(e) => {
          ...
          <div className="mt-4 grid gap-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                <label className="text-xs font-bold text-gray-500">截止（選填）</label>
                <input
                  type="datetime-local"
                  value={visibleUntil}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button
              ...
              disabled={saving}
              onClick={submit}
              className="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-indigo-700 disabled:opacity-50"
            >
              {editingId ? '儲存變更' : '發布公告'}
            </button>
            {editingId && (
        ...
        <div className="mt-8 rounded-3xl border border-gray-200 bg-white p-6 shadow-md">
          <h2 className="text-lg font-extrabold text-gray-900">目前班級：已發布</h2>
        ...
        <div className="mt-8 rounded-3xl border border-gray-200 bg-white p-6 shadow-md">
          <h2 className="text-lg font-extrabold text-gray-900">全平台公告總覽</h2>
          <p className="mt-1 text-xs text-gray-500">最近 300 筆</p>
          <ul className="mt-4 max-h-80 space-y-2 overflow-y-auto text-sm">
            {allRows.map(
              (r: {
                id: string;
                title: string;
                classGroup: { name: string; schoolCode: string };
                readCount: number;
              }) => (
                <li key={r.id} className="rounded-lg border border-gray-100 px-3 py-2">
                  <span className="font-semibold text-gray-900">{r.title}</span>
                  <span className="text-gray-500"> · {r.classGroup.name}</span>
                  <span className="text-xs text-gray-400"> · 已讀 {r.readCount}</span>
                </li>
              )
            )}
          </ul>
        ...
        >
          ...
          >
            ...
            <ul className="mt-4 space-y-2 text-sm">
              ...
              {(readsData?.reads ?? []).map(
                (r: { account: string; firstOpenedAt: string; lastOpenedAt: string; openCount: number }) => (
                  <li key={r.account} className="rounded-xl border border-gray-100 px-3 py-2">
                    ...
                    <div className="text-xs text-gray-600">
                      首次 {fmt(r.firstOpenedAt)} · 最後 {fmt(r.lastOpenedAt)} · {r.openCount} 次
                    </div>
                  </li>
                )
              )}
